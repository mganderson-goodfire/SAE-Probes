...
def get_xy_traintest_specify(num_train, numbered_dataset_tag, layer, model_name, pos_ratio=0.5, MAX_AMT=5000, seed=42, num_test = None):
...
def get_xy_traintest(num_train, numbered_dataset_tag, layer, model_name, MAX_AMT=5000, seed=42):
    X_train, y_train, X_test, y_test  = get_xy_traintest_specify(num_train, numbered_dataset_tag, layer, model_name, pos_ratio=0.5, MAX_AMT=MAX_AMT, seed=seed)
...
def get_OOD_traintest(dataset, model_name = 'gemma-2-9b', layer = 20):
    X_train, y_train, _, _ = get_xy_traintest_specify(num_train = 1024, numbered_dataset_tag = dataset, layer = layer, model_name = model_name, MAX_AMT = 1500, pos_ratio = 0.5, num_test = 0)
