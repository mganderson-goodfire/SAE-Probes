...
def generate_dataset_activations(model_name, device="cuda:0", max_seq_len=1024, OOD=False):
    os.makedirs(f"data/model_activations_{model_name}{'_OOD' if OOD else ''}", exist_ok=True)
    ...
    if model_name == "gemma-2-9b":
        hook_names = ["hook_embed"] + [f"blocks.{layer}.hook_resid_post" for layer in [9, 20, 31, 41]]
    elif model_name == "llama-3.1-8b":
        hook_names = ["hook_embed"] + [f"blocks.{layer}.hook_resid_post" for layer in [8, 16, 24, 31]]
    elif model_name == "gemma-2-2b":
        hook_names = ["hook_embed"] + [f"blocks.{layer}.hook_resid_post" for layer in [12]]
    else:
        raise ValueError(f"Model {model_name} not supported")
    ...
    for dataset_name in dataset_names:
        ...
        file_names = [f"data/model_activations_{model_name}{'_OOD' if OOD else ''}/{dataset_short_name}_{hook_name}.pt" for hook_name in hook_names]
        lengths = None
...
if __name__ == "__main__":
    ...
    if args.model_name:
        ...
        generate_dataset_activations(args.model_name, args.device, args.max_seq_len, args.OOD)
    else:
        # Run for all models
        model_names = ["gemma-2-9b", "llama-3.1-8b", "gemma-2-2b"]
        for model_name in model_names:
            print(f"Processing model: {model_name}")
            generate_dataset_activations(model_name, args.device, args.max_seq_len, args.OOD)
# %%
