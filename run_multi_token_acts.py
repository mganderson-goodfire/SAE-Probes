...
def run_sae_aggregated_probing(dataset, layer, sae_id, k, binarize=False):
    ...
    for sae_aggregation_method, save_path in zip(["mean", "max"], save_paths):
        metrics = train_aggregated_probe_on_acts(X_train_sae, X_test_sae, y_train, y_test, sae_aggregation_method, k, binarize)
...
def run_baseline_concat_probing(dataset, layer, sae_id, number_to_concat=255, pca_k=20):
    ...
    metrics = train_concat_baseline_on_model_acts(X_train_model, X_test_model, y_train, y_test, number_to_concat, pca_k)
    metrics["dataset"] = dataset
    metrics["layer"] = layer
