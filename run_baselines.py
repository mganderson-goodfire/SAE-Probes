            ...
            for method_name in methods.keys():
                ...
                if os.path.exists(savepath):
                    ...
                    if num_train + 100 <= dataset_sizes[dataset]:
                        raise ValueError(f'Missing file {method_name}, {dataset} ({num_train}/{dataset_sizes[dataset]})')
...
def run_baseline_class_imbalance(dataset_frac, numbered_dataset, method_name, model_name = 'gemma-2-9b',layer = 20):
    ...
    metrics = method(X_train, y_train, X_test, y_test)
    # Create row with dataset and method metrics and save to csv
    row = {'dataset': numbered_dataset, 'method': method_name, 'ratio': dataset_frac, 'num_train': num_train}
    for metric_name, metric_value in metrics.items():
        row[f'{metric_name}'] = metric_value
    pd.DataFrame([row]).to_csv(savepath, index=False)
    return True
...
def run_baseline_corrupt(corrupt_frac, numbered_dataset, method_name, model_name = 'gemma-2-9b',layer = 20):
    ...
    metrics = method(X_train, y_train, X_test, y_test)
    # Create row with dataset and method metrics and save to csv
    row = {'dataset': numbered_dataset, 'method': method_name, 'ratio': corrupt_frac, 'num_train': num_train}
    for metric_name, metric_value in metrics.items():
        row[f'{metric_name}'] = metric_value
    pd.DataFrame([row]).to_csv(savepath, index=False)
    return True
        ...
        for frac in fracs:
            for method_name in ['logreg']:
                ...
                if os.path.exists(savepath):
                    ...
                    raise ValueError(f'Missing file {method_name}, {dataset} ({frac}/{dataset_sizes[dataset]})')
                    #print(f'Missing file {method_name}, {dataset} ({num_train}/{dataset_sizes[dataset]})')
...
def run_datasets_OOD(model_name = 'gemma-2-9b', runsae = True, layer = 20, translation = False):
    ...
    for dataset in tqdm(datasets):
        ...
        tosave = {
            'dataset': dataset,
            'test_auc_baseline': metrics['test_auc']
        }
        if runsae:
...
def latent_performance(dataset = '66_living-room'):
    ...
    results_df = pd.DataFrame(results)
    os.makedirs(f'results/sae_probes_gemma-2-9b/OOD/OOD_latents/{dataset}', exist_ok=True)
    results_df.to_csv(f'results/sae_probes_gemma-2-9b/OOD/OOD_latents/{dataset}/{dataset}_latent_aucs_raw.csv', index=False)
...
def examine_glue_classifier():
    ...
    mask = y_test_og == 1
    confident_wrong_indices = np.argsort(prob_class_0[mask])[-5:][::-1]  # Top 5 most confident mistakes
    ...
    for idx in actual_indices:
        print(f"{prompts[idx]:<60} | {prob_class_0[idx]:.3f}     | {y_test_og[idx]:<8} | {y_test_ens[idx]:<8}")
