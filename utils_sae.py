...
def get_xy_glue_sae(toget = 'ensemble', k = 128):
    ...
    X_test = torch.load(f'data/dataset_investigate/sae_gemma-2-9b_87_glue_cola.pt', weights_only=False).to_dense().cpu()
...
def get_grammar_feature_examples():
    ...
    for idx in top_5_idx:
        print(f"{prompts[idx]:<30} | {yog[idx]:<8} | {yens[idx]:<8} | {X[idx].item():.2f}")
