...
def get_sorted_indices_new(X_train_sae, y_train):
    ...
    X_train_diff = X_train_sae_normalized[y_train == 1].mean(dim=0) - X_train_sae_normalized[y_train == 0].mean(dim=0)
    sorted_indices = torch.argsort(torch.abs(X_train_diff), descending=True)
    return sorted_indices

def get_sae_paths_wrapper(dataset, layer, sae_id, reg_type, setting, model_name="gemma-2-9b", binarize=False, num_train=None, corrupt_frac=None, frac=None):
    if setting == "normal":
        extra_string = "_"
...
def run_baseline(dataset, layer, sae_id, reg_type, setting, model_name="gemma-2-9b", binarize=False, num_train=None, corrupt_frac=None, frac=None):
    ...
    paths = get_sae_paths_wrapper(dataset, layer, sae_id, reg_type, setting, model_name, binarize, num_train, corrupt_frac, frac)
                    ...
                    if setting == "normal":
                        ...
                        for num_train in train_sizes:
                            if num_train > dataset_sizes[dataset] - 100:
                                continue
                            paths = get_sae_paths_wrapper(dataset, layer, sae_id, reg_type, setting, model_name, binarize, num_train=num_train)
                            if not os.path.exists(paths["save_path"]):
                                found_missing = True
