...
activations = torch.load(f"data/model_activations_gemma-2-9b/{dataset}_blocks.{layer}.hook_resid_post.pt")
device = "cuda:1"
...
for i in tqdm(range(len(text))):
    last_tokens.append(tokenizer(text[i], return_tensors="pt").to(device)['input_ids'][:, -1])

last_tokens = torch.cat(last_tokens, dim=0)

mean_diffs = sae_activations[targets == 1].mean(axis=0) - sae_activations[targets == 0].mean(axis=0)

# %%
import matplotlib.pyplot as plt

def plot_feature_analysis(sae_activations, targets, last_tokens, tokenizer, 
                         top_k=3, cutoff=100, figsize=(3.25, 1.6),  # Made overall figure wider
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=figsize, gridspec_kw={'width_ratios': [1.1, 1]})  # Made left plot wider
    # Plot 1: Top Features
    ...
    ax1.bar(range(len(top_diffs)), top_diffs.cpu().numpy(), alpha=bar_alpha, color='green')
    ...
    common_tokens = {t for t in set(human_token_counts) | set(ai_token_counts)
    ...
    ax2.bar(x - width/2, human_freqs, width, label='Human', alpha=bar_alpha, color='blue')
    ...
    ax2.set_xticklabels(token_labels, rotation=label_rotation_right, fontsize=ytick_fontsize)
    ax2.tick_params(axis='y', labelsize=ytick_fontsize)
    ax2.legend(fontsize=fontsize, loc='upper right', bbox_to_anchor=(1.1, 1.2), framealpha=0.9)
